...
Single-file LocalScript with embedded ~800 real English words (all >= 10 letters)
Behavior:
 - LONG_WORD_MIN = 10 (only long words)
 - Auto-type enabled (per-character via VirtualInputManager with 0.5s delay)
 - 1.2s delay between full-word sends
 - IDA-style internal naming (IDA_ prefix used for key functions/vars)
 - List loaded from GitHub (no embedded list)
]]--
...
local function IDA_fetchWords()
    local success, result = pcall(function()
        return game:HttpGet("https://raw.githubusercontent.com/Cracked691000/RAW/refs/heads/main/diction.txt")
    end)
    ...
    if success and result then
        ...
        warn("Failed to fetch word list from GitHub. Using fallback.")
        return {
...
local function IDA_MainScript()
	...
	local IDA_LONG_WORD_MIN = 10
	local IDA_CHAR_DELAY = 0.071
	local IDA_WORD_DELAY = 0.068
	...
	local function IDA_onFocusLost()
		...
		if found then
			...
			script.Parent.WordText.Text = "No ≥" .. tostring(IDA_LONG_WORD_MIN) .. " letter match. Trying relaxed search..."
			task.wait(0.4)
			local fallback = nil
			for i, v in ipairs(ENGLISH_WORDS) do
				if not IDA_tableItemExists(IDA_alreadyUsed, v) then
...
-- Animation script
local function NZFXD_fake_script()
...
-- Dragging script
local function ZBKLASR_fake_script()
	local script = Instance.new('LocalScript', UnpatchabombFrame)
...
-- Loading animation
local function FCSBTZS_fake_script()
